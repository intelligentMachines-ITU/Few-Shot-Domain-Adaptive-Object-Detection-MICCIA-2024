...
def create_dataloader(path, imgsz, batch_size, stride, opt, hyp=None, augment=False, cache=False, pad=0.0, rect=False,
    ...
    nw = 4 #min([os.cpu_count() // world_size, batch_size if batch_size > 1 else 0, workers])  # number of workers
...
def copy_paste(im, labels, p=0.5):
    ...
    for i in range(change_times-1):   
        cp_labels = random.sample(list(labels),2) 
        im = change_target(im, cp_labels) 
