...
def make_dirs(parent):
    ...
    mkdir(parent/'Rabin_tar_aug'/'images')
    #label dirs for olympus at each resolution and chucnk
    mkdir(parent/'Rabin_tar_aug'/'labels')
...
def preprocess_source_data(dir):
    ...
    obj_pres = {0:[], 1:[], 2:[], 3:[]}
    obj_absent = {0:[], 1:[], 2:[], 3:[]}
    ann_paths = os.listdir(dir)
    
    for path in ann_paths:
        annotations_file = os.path.join(dir,path)
    ...
    return object_count, less_obj, more_obj, obj_pres, obj_absent 
...
def get_target_few_data(target_lab_dir):
    ...
    ann_paths = os.listdir(target_lab_dir)
    for path in ann_paths:
        annotations_file = os.path.join(target_lab_dir,path)
        few_labs.append(annotations_file)  
    return few_labs


def copy_more_obj_data(more_obj_data):
    for file in more_obj_data:
        label = file.replace("source_train", "Rabin_tar_aug")
        shutil.copyfile(file, label) #copy label
        sor_img = get_image(file)
        dest_img = sor_img.replace("source_train", "Rabin_tar_aug")
...
def get_count_tobe_incremented(object_count, less_obj_images):
    increment_count = {0:[], 1:[], 2:[], 3:[]}
    maximum  = max(object_count.values())
    for obj in object_count.keys():
        if maximum > object_count[obj]:
            num_less = maximum - object_count[obj] #how many objects are less
            num_each_obj_incr = int(num_less / object_count[obj]) # number of times each objects needs to be augmneted
            total_incr = num_each_obj_incr * object_count[obj] # total count, might be less than maximum
            total_incr_in_each_img = int(total_incr / len(less_obj_images)) if int(total_incr / len(less_obj_images)) > 0 else 1
            times_each_obj_incr_in_img = int(num_each_obj_incr / total_incr_in_each_img)
            increment_count[obj] = [object_count[obj], times_each_obj_incr_in_img, total_incr_in_each_img]
...
def random_color_variation(image_patch, intensity_range=(0.3, 0.9), tar=False):
    upsampling_factor = 2  # Adjust as needed
    enhancer = ImageEnhance.Color(image_patch)
...
def get_augemented_data(root_dir, sor_lab_dir, tar_lab_dir,dir, blur=False):
    ...
    object_count_sor, less_obj_label_paths, objs_more_than_thres, labs_with_clas_present,_ = preprocess_source_data(sor_lab_dir)
    real_tar = get_target_few_data(tar_lab_dir)
    copy_more_obj_data(objs_more_than_thres)
    increment_stats = get_count_tobe_incremented(object_count_sor,less_obj_label_paths) # stats contain [original-count, number of image files a particular object has to be incremented, number of times one object has to be increemetd in an image]
    ...
    for less_objs_file in less_obj_label_paths:
        ...
        if blur:
            less_objs_img = blur_img(less_objs_img)
        ...
        for clas in increment_stats.keys():
            list = increment_stats[clas]# list containg the total number of each class, how much more instances needed, how much each one has to be multiplied and how many will each augment in eah less obj image
            #print(list)
            if list: #if list not empty
                incr_times = increment_stats[clas][-1] #num times this specific class has to be augmented
                incr_files = increment_stats[clas][1]
                ...
                if blur:
                    img_with_objs = blur_img(img_with_objs)
                ...
                for i in range(len(objs_ann)):
                    if int(objs_ann[i][0]) == clas:
                        ann = objs_ann[i]
                        if path_lab_with_obj in objects_incremented.keys():
                            dict = objects_incremented[path_lab_with_obj]
                            if clas not in dict.keys():
                                ...
                                objects_incremented[path_lab_with_obj][clas] = {i:1}
                                break
                                ...
                                if objects_incremented[path_lab_with_obj][clas][i] < incr_files:
                                    ymin,ymax,xmin,xmax = int(ann[2]), int(ann[4]) , int(ann[1]), int(ann[3])
                                    if ymin > ymax:
                                       temp = ymax
                                       ymax = ymin
                            ...
                            objects_incremented[path_lab_with_obj] = {}
                            objects_incremented[path_lab_with_obj][clas]={i:1}
        ...
        #save the new image and its annotation
        new_lab_file = less_objs_file.replace("source_train", "Rabin_tar_aug")
        new_img_file = path_img_with_less_obj.replace("source_train", "Rabin_tar_aug")
...
if __name__ == '__main__':
    ...
    root_dir = Path("datasets/raabin/rabin")
    sor_dir = Path('datasets/raabin/rabin/source_train/labels')
    tar_dir = Path('datasets/raabin/rabin/rabin_few_random2/labels')
    blur = False
